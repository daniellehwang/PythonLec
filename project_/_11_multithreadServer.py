...
clientList = [] # 클라이언트 connSock 를 저장할 리스트

def sendAll(msg, exceptSock):
    "exceptSock 을 제외하고, 연결된 모든 클라이언트에 데이터를 전송"
    for clientSock in clientList:
        if clientSock != exceptSock:
            clientSock.send(msg.encode())

class RequestHandler(StreamRequestHandler): #상속을 받는다
    # 클라이언트 접속시 스레드가 생성되고
    # 해당 스레드는 handle 메서드를 동작시킨다
    def handle(self):
        print("Connection From ", self.client_address)
        connSock = self.request     # 클라이언트와 연결된 소켓; 연결 socket
...
if __name__ == '__main__':
    # ('', PORT) : 현재 실행 프로세스가 동작하는 Host ip를 자동 할당해라
    #             9000포트 설정
    # 새로운 클라이언트가 접속할 때마다 스레드를 1개씩 할당하고
    # 해당 스레드가 RequestHandler 객체의 handle 메서드를 처리하도록 하라
    server = ThreadingTCPServer(('', PORT), RequestHandler) # thread 객체
    print('Server Start ', PORT)
    server.serve_forever()     # 서버가동 시작
